		...
		function _send_otf_request(sensor){
			...
			return new Promise((top_fulfill, top_reject) => {
				...
				setTimeout(() => {
					...
					// node.config_gateway.config_enter_otf_mode(config.addr).then((f) => {
					// 	// #OTF
					// 	node.send({topic: 'Enter OTF Result', payload: {'OTF': 'success'}});
					// 	top_fulfill(f);
					// }).catch((err) => {
					// 	node.send({topic: 'Enter OTF Result', payload: {'OTF': 'failed'}});
					// });
			...
			this.pgm_on('sensor_mode', (sensor) => {
				...
				if(sensor.type == config.sensor_type){
					...
					if(config.auto_config && sensor.mode == 'PGM'){
						...
						console.log('Inside Original FLY 2');
						_send_otf_request(sensor);
					}else if(config.auto_config && config.on_the_fly_enable && sensor.mode == "OTN"){
						console.log('Inside Original OTN 2');
						_config(sensor, true);
					}
				}
			});
